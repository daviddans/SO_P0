...
void inputHandler(char * input, bool * onRunTime, tList* listaComandos, tList* listaFicheros, tList* memory, tList* proceslist, int* control, char** enviro)
{
	...
	else if(strcmp(input,"showenv")==0) doShowEnv(args, enviro);
	else if(strcmp(input,"fork")==0) doFork(proceslist);
	else if(strcmp(input,"exec")==0) doExec(args,&args_ptr);
	else if(strcmp(input,"jobs")==0) doJobs(proceslist);
	else if(strcmp(input,"deljobs")==0) doDelJobs(args,&args_ptr, proceslist);
		...
		if(*control<MAX_REC){ //Comprobamos no exceder nuestro limite recursivo
			...
			if(args == NULL){ //Si no hay argumentosclear
				...
				if(pos == NULL) printf("No se encontro el comando: %s\n", args); // Si la posición es nula mostramos un error
					...
					inputHandler(input, onRunTime, listaComandos, listaFicheros, memory, proceslist, control, enviro); //Llamada recursiva
				}
			}
		}
		else puts("Demasiada recursion!!!\n"); // Maostramos un error si nos excedemos en el numero de recursiones
	} 
	else if(strcmp(input,"\n")!=0)doCommand(input,args,&args_ptr,proceslist);
...
int main(int argc, char **argv, char **enviro){
	...
	tList procesList; //Lista para los procesos en background
	...
	createEmptyList(&procesList); //Inicialización lista
	...
	while(onRunTime){
		...
		if(input != NULL){
			...
			inputHandler(input, &onRunTime, &listaComandos, &listaFicheros, &memory, &procesList, &control, enviro);//Procesar entrada
		}
	}
	//Borrado de las listas
	deleteListCMD(&listaComandos);
	deleteListFile(&listaFicheros);
	freeAllBGList(&procesList);
