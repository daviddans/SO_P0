...
tBGProc* newBGproc(int pid, char cmd[], char wstatus[], int signal){//Crea un bloque de memoria 
...
bool insertBGProc(tList* lista, tBGProc* process){ //Añade un bloque de memoria previamente creado a la lista
    ...
    if(process == NULL) printf("Error: No se puede añadir a la lista\n");
        ...
        if((pos = malloc(sizeof(struct node)))==NULL) r = false; //Comprobamos que la reserva de memoria sea correcta
            ...
            pos->data = process; //Guardamos los datos
            pos->next = NULL;
            if(isEmptyList(*lista)) *lista = pos; //Si la lista es vacia insertamos como unico elemento
            else{//Si no insertamos al principio (mayor eficiencia)
                pos->next = *lista;
...
tPos searchByPID(tList lista, pid_t pid){ //Devuelve la posicion de la primera coincidencia
    ...
    if(!isEmptyList(lista)){ // comprobamos que la lista no este vacia
        proc = i->data;
        while(i != NULL && proc->pid != pid){ // recorremos la lista
            i = i->next;
            if(i!=NULL)proc = i->data;
        }
        if(proc->pid == pid){ // Si encontramos el elemento devolvemos su puntero
            return i; 
        }
    }
    return NULL; //Si no encontramos el elemento devolvemos NULL
